...
class Slot:
    ...
    def scraping_slots(self):
        self.driver.get(f"{os.environ['RAGNAROK_URL']}/database/thor/itens-slot?page=1")

        WebDriverWait(self.driver, 120).until(ec.presence_of_element_located((By.XPATH, '//h1[text()="Itens Slot"]')))
        ...
        for actual_page in range(2, last_page + 1):
            items = self.driver.find_elements(By.XPATH, '//li[@class="itens-slot show"]//a')
            ...
            self.driver.get(f"{os.environ['RAGNAROK_URL']}/database/thor/itens-slot?page={actual_page}")
    ...
    def scraping_and_post(self, url):
        ...
        response = requests.post(f"http://{os.environ['APP_URL']}:{int(os.environ['APP_PORT'])}/slots", data=json.dumps(data))

        if response.status_code == 409:
            response = requests.put(f"http://{os.environ['APP_URL']}:{int(os.environ['APP_PORT'])}/slots/{data['id']}", data=json.dumps(data))
